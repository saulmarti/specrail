...
function frontendAtBuilder(root){
  ...
  const task=createTask(root,{title:'Redesign hero hierarchy',type:'task',surfaces:['frontend'],size:'small',risk:'low'});startRefinement(root,task.meta.id);let t=loadTask(findTask(root,task.meta.id));
  for(const[h,v]of [['Need','Redesign the hero visual hierarchy to reduce heading dominance while preserving content.'],['Product Value','Visitors scan the primary action more easily.'],['Users','Desktop and mobile visitors.'],['Scope','Change only the homepage hero heading presentation.'],['UI Target','- Route: `/`\n- Target: `section#homepage-hero`\n- Viewport: `1440x1000`\n- Capture: focused section'],['Out of Scope','Navigation, footer, backend, and copy.'],['Acceptance Criteria','- The hero heading and CTA remain fully readable without clipping or overflow.'],['UX/UI Proposal','Implement the approved visual proposal exactly on the hero target.'],['Implementation Plan','Apply the approved proposal and verify the same target in the served app.']])t.body=setSection(t.body,h,v);
