...
test('material execution problems create a native blocker questionnaire', () => {
    const root = repo();
    ...
    task.meta.status = 'active';
    task.meta.phase = 'builder';
    saveTask(task);
    ...
    assert.equal(loadTask(findTask(root, created.meta.id)).meta.phase, 'builder');
});
