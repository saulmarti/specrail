...
function versionTuple(value: string): number[] {
  return String(value??'').replace(/^v/i,'').split('.').slice(0,3).map(part=>Number(part.replace(/[^0-9].*$/,''))||0);
}
...
export function minimalismRequirement(role: string, capability: PonytailCapability = {available:false}, options:{disabled?:boolean}={}): MinimalismRequirement {
  const normalizedRole=String(role??'').trim().toLowerCase();
  const required=ponytailRequiredForRole(normalizedRole)&&options.disabled!==true;
  if(!required)return{required:false,provider:PONYTAIL_PROVIDER,mode:PONYTAIL_DEFAULT_MODE,role:normalizedRole,satisfied:true,reason:options.disabled===true?'Ponytail explicitly disabled for this work item.':'Role does not mutate production code.'};
  ...
  return{required:true,provider:PONYTAIL_PROVIDER,mode:PONYTAIL_DEFAULT_MODE,role:normalizedRole,satisfied,reason:satisfied?`Ponytail ${version} is attested in ${mode} mode.`:`Code mutation requires ${PONYTAIL_PROVIDER} >= ${PONYTAIL_MIN_VERSION} in full mode; no qualifying host capability is attested.`};
}

export function assertPonytailForMutation(role:string,capability:PonytailCapability,options:{disabled?:boolean}={}):void{
  const requirement=minimalismRequirement(role,capability,options);
