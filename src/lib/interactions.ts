...
function presentationHostActions(presentation: Presentation): HostActionInteraction | null {
 ...
 const actions=presentation.presentationContract.fallback.requiredHostActions.filter(action=>retryIds.has(action.id)&&action.type==='present-image');
...
export function interactionForTask(root:string,id:string,kind='current',input:InteractionInput={}):InteractionResult{
 ...
 if(kind==='final-approval'){
  const presentation=finalPresentation(root,id,sessionId);const hostActions=hasUserWaiver(root,task.meta.id,'host-presentation')?null:presentationHostActions(presentation);if(hostActions)return hostActions;
  return{tool:'request_user_input',presentation,questions:[{id:'final-approval',header:'Resultado final',question:`${presentation.markdown}\n\nDespués de revisar el resultado y las evidencias mostradas arriba para ${task.meta.id} — ${task.meta.title}, ¿aceptas el resultado?`,options:[option('Aprobar resultado','Aceptar el resultado y continuar a su entrega'),option('Solicitar cambios','Volver a la fase adecuada con comentarios'),option('Mantener abierta','Dejarla pendiente de validación')],isOther:true}]};
 }
