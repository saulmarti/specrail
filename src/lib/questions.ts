...
export function addQuestion(root:string,id:string,input:AddQuestionInput): TaskQuestion {
  ...
  const question:TaskQuestion={id:`Q-${String(number).padStart(3,'0')}`,status:'open',category:input.category||'general',impact:input.impact||'medium',text:input.text,options:input.options||[],recommendation:input.recommendation||null,answer:null,created_at:new Date().toISOString(),answered_at:null};
  ...
  appendLog(task,`Question ${question.id} added; workflow waits for user.`);const saved=saveTask(task);recordTrace(root,saved,'question-added',{questionId:question.id,category:question.category,impact:question.impact});return question;
}
export function answerQuestion(root:string,id:string,questionId:string,answer:string): ReturnType<typeof saveTask> {
  ...
  question.status='answered';question.answer=String(answer);question.answered_at=new Date().toISOString();task.body=setSection(task.body,'Questions',render(questions));task.meta.open_questions=questions.filter(q=>q.status==='open').length;
  if(task.meta.open_questions===0){task.meta.waiting_for='none';if(task.meta.status==='blocked'&&task.meta.resume_status){task.meta.status=task.meta.resume_status;task.meta.phase=task.meta.resume_phase??'product-specifier';task.meta.resume_status=null;task.meta.resume_phase=null;}}
