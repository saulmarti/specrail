...
export function lintSpecification(task:TaskDocument,{stage='approval'}:{stage?:string}={}):SpecificationLint{
 ...
 const frontend=task.meta.surfaces.some(x=>['frontend','ui','ux'].includes(x))||task.meta.route.design;if(frontend){const target=sections['UI Target']??'';if(!target)errors.push('UI Target is required for frontend/UI work');else{if(!/(?:route|ruta|screen|pantalla)\s*:/i.test(target))errors.push('UI Target must identify the route or screen');if(!/(?:target|selector|objetivo|anchor|ancla|section|componente)\s*:/i.test(target))errors.push('UI Target must identify the exact element or visible anchor');if(!/(?:capture|captura)\s*:/i.test(target))warnings.push('UI Target should define the capture scope');if(!/(?:viewport|mobile|desktop|m[oó]vil|escritorio)\s*:/i.test(target))warnings.push('UI Target should define affected viewports');}}
