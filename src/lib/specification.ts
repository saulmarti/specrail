...
import { isMicroControl } from './control-profile.js';
...
const GOVERNED_META_V1 = ['title','type','size','risk','execution_profile','workflow_mode','fast_authorized_at','surfaces','route','file_scope','dependencies','parent_id'] as const;
...
export function lintSpecification(task:TaskDocument,{stage='approval'}:{stage?:string}={}):SpecificationLint{
 ...
 const requiredSections=isMicroControl(task)?['Need','Scope','Out of Scope','Acceptance Criteria']:['Need','Product Value','Scope','Out of Scope','Acceptance Criteria'];for(const name of requiredSections)if(!sections[name])errors.push(`${name} is required`);
