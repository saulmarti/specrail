...
export function completePhase(root: string, id: string, options: Record<string, unknown> = {}) {
  ...
  if(executionPhase(task.meta.phase)){ensureApprovedSpecUnchanged(root,task);if(task.meta.phase==='builder'||task.meta.phase==='technical-reviewer')assertPhaseBoundaryEntered(root,task.meta.id,task.meta.phase,session(options));assertTaskLease(root,task.meta.id,{sessionId:session(options)??undefined});}
  ...
  switch(task.meta.phase){
    ...
    case 'builder': { const next=nextAfterBuilder(task);if(next==='technical-reviewer')invalidatePhaseBoundary(root,task.meta.id,'technical-reviewer');task.meta.phase=next;task.meta.status=statusForPhase(task.meta.phase);appendLog(task,`Builder completed; next phase: ${task.meta.phase}.`);break; }
  ...
  if((completedPhase==='builder'&&saved.meta.phase==='technical-reviewer')||saved.meta.waiting_for==='user'||['awaiting_spec_approval','awaiting_final_approval'].includes(saved.meta.status))releaseTaskLease(root,saved.meta.id,{sessionId:session(options)??undefined,force:true});return saved;
}
