...
export const ACTIVATION_BODY=[
  ...
  '',
  'At approval gates use a stable session token and the exact SpecRail interaction; never invent request_user_input. For visual gates, interaction.tool=host_actions means: show the complete Review Bundle, execute exact actions, record real outcomes with presentationDigest/action ID, then call next again. Required visuals belong in the conversation; local paths never count as shown evidence and failed/unavailable required images block approval. Generated Cockpit HTML is not proof of display: record opened only after a real open, otherwise expose exact openUrl as a clickable action and record offered. Never pre-acknowledge or reuse stale, cross-session, or tampered acknowledgments. $visualize preparation leaves hostPresentation unverified. Frontend preview uses served presentation.previewUrl, never raw index.html/file://. Shell localhost failure differs from host Browser failure/unavailability. Codex owns model/effort selection. Obey `next.autonomy` and its recommended specialist skill. At phase boundaries persist the native choice, end the turn, then on Continue/Resume enter before generic Kanban/memory/process reads. Never implement before specification approval or finish before final approval and delivery.'
].join('\n');
