...
export const PI_ACTIVATION_BODY=[
  ...
  '',
  'For NEW delivery work without a route prefix or TASK continuation, call `specrail_entry_gate` before SpecRail state. `Sin SpecRail:` / `No SpecRail:` selects Direct; `Directo + verificar:` selects Direct+Verify; `SpecRail Fast:` selects governed fast mode. Direct routes create no SpecRail workflow state.',
  '',
  'After SpecRail selection call `specrail_skill` with exact name `ai-flow`; it is the complete workflow contract. Use `specrail_cli` for SpecRail argv, `specrail_host_context` for the Brain session, `specrail_codegraph` for structural context, and `specrail-worker` for routed heavy work. `next` failure blocks; diagnostics never replace `next`. Never assume material decisions; ask 2-4 choices plus free text when evidence is not unique. SpecRail loads the official `@dietrichgebert/ponytail` Pi extension/skills; code-writing requires `full`. Missing Ponytail means the SpecRail install is broken. Prefer attested ask_user_question; otherwise forward exact request_user_input. Before approval show one concise Decision Capsule in chat; do not generate or open Review Cockpit. Required visual evidence remains inline. The current Pi model remains Brain; Workers use explicit isolated model-pinned processes. Fresh boundaries use `/specrail-handoff TASK-####` or `/new` + `Continue TASK-####`.'
].join('\n');
