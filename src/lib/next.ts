...
export function nextAction(root:string,id:string,options:NextOptions={}){
 ...
 const phaseSkill=skillForPhase(root,task.meta.phase);
 const phaseIntelligence=phaseSkill?intelligenceRecommendation(task,{actor:phaseSkill,action:'continue',recommendedSkill:phaseSkill}):intelligence;
 return{task:task.meta.id,status:task.meta.status,projectContext:projectContext.status,phase:task.meta.phase,workflowMode:task.meta.workflow_mode||'standard',fastActive,controlProfile:task.meta.route.control_profile||null,controlReasons:Array.isArray(task.meta.route.control_reasons)?task.meta.route.control_reasons:[],intelligence,phaseIntelligence,autonomy:{...policy,...autonomyDecision},productIntelligence:{context:productContext,productOwner,finalProductOwner,audience},concurrency,concurrencyAuthority,readiness,actor,recommendedSkill,action,userInputRequired,dependencies:deps,interaction,evalCandidates,activeEvals:activeEvals.map(item=>({id:item.id,category:item.category,statement:item.statement,path:item.path})),lease,context:contextInfo,evidence,runtime,visualization};
}
