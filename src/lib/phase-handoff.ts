...
export function runtimeRecommendation(root: string, id: string, options: { sessionId?: string | null } = {}): RuntimeRecommendation {
  ...
  const transitionNotice = sessionEntryRequired && handoff
    ? {
        ...
        title: transferRequired ? 'Phase ownership must be entered in this host session' : role === 'target-audience' ? 'Fresh Target Audience session required' : boundary?.status==='chosen' ? 'Phase boundary choice recorded; entry required' : role === 'implementer' ? 'Implementation boundary ready' : 'Independent review boundary ready',
        message: transferRequired
          ? `${task.meta.id} is already inside the ${role === 'implementer' ? 'implementation' : role === 'target-audience' ? 'Target Audience' : 'review'} phase, but that phase boundary belongs to another host session (${boundary?.enteredSessionId || 'unknown'}). Do not continue phase work from this chat until this session explicitly enters the boundary; an active lease may require a user-approved takeover.`
          : role === 'target-audience'
          ...
          : role === 'implementer'
            ? `Planning is sealed for ${task.meta.id}. End this turn before coding. Next turn you may continue in this chat or open a fresh host session/chat; SpecRail does not choose or store a model; the host model/reasoning selector remains authoritative. ${boundary?.recommendation === 'fresh-chat-recommended' ? 'A fresh chat is recommended because it removes the planning conversation from implementation context and lets a less-capable implementer start from the compiled capsule.' : 'This is a small low-risk task, so continuing in the same chat is reasonable; a fresh chat remains available for stronger isolation.'}`
            : `Implementation is sealed for ${task.meta.id}. End this turn before independent review. Next turn you may continue in this chat or open a fresh host session/chat. ${boundary?.recommendation === 'fresh-chat-recommended' ? 'A fresh chat is recommended to remove builder assumptions from reviewer context.' : 'For this small low-risk change, same-chat review is acceptable after entering the review boundary.'}`,
  ...
  return {
    ...
    rationale: role === 'thinker'
      ? `Keep planning/refinement on a bounded ${contextProfile} repository context. The model and reasoning setting remain whatever the user selected in the host.`
      : role === 'implementer'
    ...
    transitionInstruction: transitionNotice && handoff
      ? role === 'target-audience'
        ...
        : boundary?.status==='chosen'
          ? `The native phase-boundary choice is already persisted as ${boundary.choice}. Do not ask it again. Before reading generic repository process files or doing ${role === 'implementer' ? 'implementation' : 'independent review'} work, enter the phase boundary in this host session, then treat ${handoff.relativePath} as the compiled phase contract. Previous conversational reasoning is non-authoritative. Do not replay the previous chat and do not reread generic Kanban/memory/process files unless the capsule names them, a concrete conflict requires source verification, or a specific missing implementation detail requires them.`
          : `STOP before doing ${role === 'implementer' ? 'implementation' : 'independent review'} work in this turn. Show the transition notice once, persist the exact native boundary choice, and end the turn. If the user chooses a fresh chat, use the host-native fresh-session mechanism. 'transitionNotice.freshChatUrl' is an optional Codex convenience when supported; other adapters use the same 'resumePrompt' with their own fresh-session action. On the next user turn or in the selected fresh chat, enter the phase boundary first. Then treat ${handoff.relativePath} as the compiled phase contract. Previous conversational reasoning is non-authoritative. Do not replay the previous chat and do not reread generic Kanban/memory/process files unless the capsule names them, a concrete conflict requires source verification, or a specific missing implementation detail requires them.`
