...
assert.equal(verificationCommandAllowed('npm', ['test']), true);
assert.equal(verificationCommandAllowed('sh', ['-c', 'npm test && rm -rf x']), false);
...
assert.deepEqual(decisionBlockers([{ id: 'api', material: true, status: 'resolved', source: 'active_user', ref: 'turn-1' }]), []);
...
for (const required of ['specrail_ponytail', 'specrail_ponytail_review_result', 'specrail_mutation_gate', 'specrail_verify']) assert(tools.has(required), `missing ${required}`);
...
const ctx = {
  cwd: process.cwd(),
  hasUI: true,
  ui: { async select() { return 'Keep Ponytail'; }, notify() {} },
...
await tools.get('specrail_mutation_gate').execute('g1', { reviewed: true, decisions: [], noMaterialDecisionsReason: 'The requested change is fully explicit.' }, undefined, undefined, ctx);
...
assert.equal(last.route, null, 'Direct route must reset after successful work-item completion');
