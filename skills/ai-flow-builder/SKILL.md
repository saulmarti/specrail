...
1. **Ponytail is required for production-code mutation.** Activate the official `ponytail` skill/plugin in `full` mode before editing. Do not imitate or paraphrase Ponytail and claim it is active. If the host cannot attest Ponytail, stop before mutation and use structured user input to either install/enable the official plugin or explicitly disable Ponytail for this work item. Never install third-party code silently. SpecRail scope, security, accessibility, data-loss protection, acceptance criteria, QA evidence, and explicit user requirements take precedence over minimalism.
