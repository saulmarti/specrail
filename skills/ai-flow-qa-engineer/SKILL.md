...
3. Frontend: use the running application through served `http://`/`https://` (never `file://` or raw `index.html`) and obey `control_profile`: `light` performs focused Before→After verification plus DOM/layout audit; `standard/rigorous` compare Before→approved Proposal→After; `micro` has no separate QA phase. Register the exact runtime URL with the after evidence and keep it reachable through the final review gate. First distinguish the current coding-agent host surface from the shell sandbox. When the current host exposes an attested Browser capability/plugin, invoke that host Browser against the localhost URL even if terminal `curl` cannot reach it. A host or surface without Browser capability is a different `unavailable` reason from a browser connection failure. Never infer host-browser failure from a shell failure.
...
7. Use `$visualize` only for an interactive multi-flow/device evidence map when the active host exposes the installed Codex Visualize skill and it improves review; canonical evidence remains authoritative. A prepared Visualize artifact/reference does not prove host display, so keep the direct canonical-evidence fallback when presentation is unverified. Never ask the user to type `/visualize` and never invoke the plugin's internal renderer directly.
...
Capture the same target, capture scope, and pixel viewport used by the profile's approved visual evidence from the live served runtime URL. `light` requires the matching Before context; `standard/rigorous` require matching Before/Proposal context. A page-top or unrelated screenshot is invalid. Register `ui-after-validation` from real browser/DevTools measurements before approval.
