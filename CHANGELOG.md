...
- Added `specrail update`, preserving the installed `beta`/`latest` channel by default, supporting explicit `--beta` / `--latest` switching and network-free `--dry-run`, and refreshing managed Codex + Pi assets with the newly installed CLI rather than an older managed copy.
